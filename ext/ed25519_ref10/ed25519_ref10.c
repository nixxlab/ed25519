...
static VALUE mEd25519_Provider_Ref10_verify(VALUE self, VALUE verification_key, VALUE signature, VALUE msg);
...
static VALUE mEd25519_Provider_Ref10_create_keypair(VALUE self, VALUE seed)
{
    uint8_t verification_key[PUBLICKEYBYTES];
    ...
    crypto_sign_ed25519_ref10_seed_keypair(verification_key, keypair, (uint8_t *)RSTRING_PTR(seed));
...
static VALUE mEd25519_Provider_Ref10_verify(VALUE self, VALUE verification_key, VALUE signature, VALUE msg)
{
    ...
    StringValue(verification_key);
    StringValue(signature);
    StringValue(msg);

    if(RSTRING_LEN(verification_key) != PUBLICKEYBYTES) {
    ...
    result = crypto_sign_open_ed25519_ref10(
        buffer, &buffer_len,
        sig_and_msg, sig_and_msg_len,
        (uint8_t *)RSTRING_PTR(verification_key)
    );
